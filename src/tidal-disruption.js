...
export function tidalStreamStrength(target,hole,distanceAU){
 if(!target||!hole||target.key==='blackhole'||!(distanceAU>0))return 0;
 const tidalRadius=tidalRadiusAU(target,hole),start=tidalRadius*11;
 return Math.max(0,Math.min(1,(start-distanceAU)/Math.max(start-tidalRadius*.8,1e-20)));
}
