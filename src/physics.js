...
export const moons=[
 ...
 ['Tryton','neptune',354759,2.139e22,1353.4,-5.877,false],['Proteusz','neptune',117647,4.4e19,210,.112,true],['Nereida','neptune',5513400,3.1e19,170,360.13,true]
];
let nextId=0;
...
export function mergeCollisions(bs){const removed=[];for(let i=0;i<bs.length;i++)for(let j=bs.length-1;j>i;j--){const a=bs[i],b=bs[j],r=Math.hypot(...a.p.map((x,k)=>x-b.p[k]));const ra=a.key==='blackhole'?2*G*a.mass/(173.1446**2):a.radius/AU,rb=b.key==='blackhole'?2*G*b.mass/(173.1446**2):b.radius/AU;if(r<ra+rb){const m=a.mass+b.mass,large=a.mass>=b.mass?a:b,small=large===a?b:a;for(let k=0;k<3;k++){large.v[k]=(a.v[k]*a.mass+b.v[k]*b.mass)/m;large.p[k]=(a.p[k]*a.mass+b.p[k]*b.mass)/m}large.mass=m;if(large.key!=='blackhole')large.radius=Math.cbrt(a.radius**3+b.radius**3);removed.push(small.id);if(small===a){bs[i]=b;}bs.splice(j,1);}}return removed}
