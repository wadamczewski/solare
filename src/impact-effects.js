...
export function createImpactEffects(scene){
 ...
 function add(event,position,size){
  ...
  for(let i=0;i<count;i++){const z=1-2*(i+.5)/count,theta=i*2.399963,r=Math.sqrt(1-z*z),direction=new THREE.Vector3(r*Math.cos(theta),z,r*Math.sin(theta));seeds.push({direction,theta,variation:.4+(i%31)/20});positions.set(direction.clone().multiplyScalar(size*.8).toArray(),i*3);const color=new THREE.Color().setHSL(.035+(i%17)/240,.8,.55+(i%7)/20);colors.set(color.toArray(),i*3);sizes[i]=1.5+(i%11)*.35;brightness[i]=1;}
...
float r=length(gl_PointCoord-.5)*2.;if(r>1.)discard;gl_FragColor=vec4(tint,alpha*exp(-r*r*5.));
  ...
  const flash=new THREE.Mesh(new THREE.SphereGeometry(1,24,16),new THREE.MeshBasicMaterial({color:'#fff0cf',transparent:true,opacity:.7,depthWrite:false,blending:THREE.AdditiveBlending,toneMapped:false}));flash.scale.setScalar(size);group.add(flash);
  const wave=new THREE.Mesh(new THREE.SphereGeometry(1,32,20),new THREE.MeshBasicMaterial({color:'#ffad66',transparent:true,opacity:.12,wireframe:true,depthWrite:false,blending:THREE.AdditiveBlending}));wave.visible=!absorb;group.add(wave);
  const light=new THREE.PointLight('#ff9c52',0,0,0);group.add(light);
  active.push({group,dust,shards,flash,wave,light,seeds,size,age:0,absorb,survivor:event.survivor,p:[...event.p],v:[...event.v],duration:absorb?7:event.kind==='graze'?3.5:6});
 }
 function update(dt,map,simulationDelta,findSurvivor){for(let i=active.length-1;i>=0;i--){const e=active[i];e.age+=dt;e.p=e.p.map((x,k)=>x+e.v[k]*simulationDelta);const anchor=e.absorb?findSurvivor?.(e.survivor):null;e.group.position.copy(anchor||map(e.p));const t=e.age/e.duration;if(t>=1){dispose(e);active.splice(i,1);continue}
  ...
  for(let j=0;j<count;j++){const seed=e.seeds[j],local=Math.max(0,Math.min(1,t*1.25-(j%13)*.015));let p;
   ...
   else{p=seed.direction.clone().multiplyScalar(e.size*(.65+4.5*local*seed.variation));p.y+=Math.sin(seed.theta)*e.size*local*.3;}
   a.setXYZ(j,p.x,p.y,p.z);b.setX(j,(1-local)**1.4*(.7+.3*Math.sin(j+t*18)**2));
  }a.needsUpdate=true;b.needsUpdate=true;
  for(let j=0;j<shardCount;j++){const index=j*29%count;dummy.position.fromBufferAttribute(a,index);dummy.rotation.set(t*(j%3+1)*4,t*5+j,t*3);dummy.scale.set(e.size*.055*(1-t),e.size*.035*(1-t),e.size*(e.absorb?.16:.08)*(1-t));dummy.updateMatrix();e.shards.setMatrixAt(j,dummy.matrix)}e.shards.instanceMatrix.needsUpdate=true;e.shards.material.opacity=(1-t)**.7;e.shards.material.emissiveIntensity=2.5*(1-t)**2;
