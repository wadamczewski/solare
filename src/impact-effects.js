...
// A Chicxulub-scale impact forms a small, fast ejecta curtain at the surface.
// It does not throw planet-sized chunks through the target, which was making a
...
export function createImpactEffects(scene){
 const active=[],maxEvents=8,count=1024;
 const dummy=new THREE.Object3D();
 function dispose(e){e.group.removeFromParent();e.group.traverse(o=>{o.geometry?.dispose();o.material?.dispose()})}
 // A surviving impact belongs to the struck surface. The parent mesh carries
 // its translation, axial rotation and scale, so ejecta and the pressure wave
 // cannot be left behind in world space while the planet continues its orbit.
 function add(event,position,size,{surfaceMesh=null,surfaceAxis=null,surfaceRadius=0}={}){
  if(active.length>=maxEvents)dispose(active.shift());const profile=impactVisualProfile(event),attached=profile.planetaryImpact&&surfaceMesh&&surfaceAxis&&surfaceRadius>0,group=new THREE.Group();
  const normal=attached?surfaceAxis.clone().normalize():new THREE.Vector3(...(event.impactDirection||event.normal||[0,1,0])).normalize();
  if(attached){surfaceMesh.add(group);group.position.copy(normal).multiplyScalar(1.012);size/=surfaceRadius}else{group.position.copy(position);scene.add(group)}
  ...
  const tangent=new THREE.Vector3().crossVectors(normal,Math.abs(normal.y)<.9?new THREE.Vector3(0,1,0):new THREE.Vector3(1,0,0)).normalize(),bitangent=new THREE.Vector3().crossVectors(normal,tangent).normalize();
  ...
  const material=new THREE.ShaderMaterial({transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,toneMapped:false,uniforms:{dpr:{value:Math.min(globalThis.devicePixelRatio||1,2)}},vertexShader:`
#include <common>
...
float r=length(gl_PointCoord-.5)*2.;if(r>1.)discard;gl_FragColor=vec4(tint,alpha*exp(-r*r*5.));
  ...
  const shards=new THREE.InstancedMesh(new THREE.IcosahedronGeometry(1,0),new THREE.MeshStandardMaterial({color:'#8a5a3a',emissive:'#ff6b20',emissiveIntensity:2,roughness:1,transparent:true}),SHARD_COUNT);shards.instanceMatrix.setUsage(THREE.DynamicDrawUsage);shards.frustumCulled=false;group.add(shards);
  ...
  const effect={group,dust,shards,flash,wave,light,seeds,normal,size,energy:Math.min(1,Math.max(0,event.energy||0)),age:0,absorb,attached,survivor:event.survivor,p:[...event.p],v:[...event.v],surfaceAnchor:position.clone(),...profile};active.push(effect);return effect;
 }
 function update(dt,map,simulationDelta,findSurvivor){for(let i=active.length-1;i>=0;i--){const e=active[i];e.age+=dt;e.p=e.p.map((x,k)=>x+e.v[k]*simulationDelta);const anchor=e.absorb?findSurvivor?.(e.survivor):null;if(!e.attached)e.group.position.copy(e.planetaryImpact?e.surfaceAnchor:anchor||map(e.p));const t=e.age/e.duration;if(t>=1){dispose(e);active.splice(i,1);continue}
