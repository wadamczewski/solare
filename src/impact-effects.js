...
export function createImpactEffects(scene){
 ...
 function add(event,position,size){
  ...
  const profile=impactVisualProfile(event),absorb=event.kind==='absorb',geometry=new THREE.BufferGeometry(),positions=new Float32Array(count*3),colors=new Float32Array(count*3),sizes=new Float32Array(count),brightness=new Float32Array(count),seeds=[];
  const normal=new THREE.Vector3(...(event.impactDirection||event.normal||[0,1,0])).normalize(),tangent=new THREE.Vector3().crossVectors(normal,Math.abs(normal.y)<.9?new THREE.Vector3(0,1,0):new THREE.Vector3(1,0,0)).normalize(),bitangent=new THREE.Vector3().crossVectors(normal,tangent).normalize();
...
float r=length(gl_PointCoord-.5)*2.;if(r>1.)discard;gl_FragColor=vec4(tint,alpha*exp(-r*r*5.));
  ...
  active.push({group,dust,shards,flash,wave,light,seeds,normal,size,energy:Math.min(1,Math.max(0,event.energy||0)),age:0,absorb,survivor:event.survivor,p:[...event.p],v:[...event.v],surfaceAnchor:position.clone(),...profile});
 }
 function update(dt,map,simulationDelta,findSurvivor){for(let i=active.length-1;i>=0;i--){const e=active[i];e.age+=dt;e.p=e.p.map((x,k)=>x+e.v[k]*simulationDelta);const anchor=e.absorb?findSurvivor?.(e.survivor):null;e.group.position.copy(e.planetaryImpact?e.surfaceAnchor:anchor||map(e.p));const t=e.age/e.duration;if(t>=1){dispose(e);active.splice(i,1);continue}
