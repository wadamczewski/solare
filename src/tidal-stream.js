...
export function tidalSpiralPoint(start,end,strength,time,phase,u){
 ...
 const base=start.clone().lerp(end,u),turns=2.4+strength*6.5;
 const angle=phase+u*Math.PI*2*turns-time*(2.2+strength*6.6);
 const envelope=Math.pow(Math.sin(Math.PI*Math.min(.985,u)),.45)*Math.pow(1-u,.12);
 const radius=distance*(.018+strength*.19)*envelope;
