...
function releaseSurfaceOrientation(){
 for(const view of views.values()){if(view.surfaceOriented){view.axis.quaternion.identity();view.surfaceOriented=false}}
}
...
function updateEducationLayer(){
 ...
 const body=bs.find(item=>item.id===educationSubjectId)||bs.find(item=>item.id===(selected??follow))||bs.find(item=>item.key==='earth')||bs[0];
