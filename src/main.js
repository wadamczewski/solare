...
const composer=new EffectComposer(renderer);composer.addPass(new RenderPass(scene,camera));const solarBloom=new UnrealBloomPass(new THREE.Vector2(innerWidth,innerHeight),.65,.45,2);composer.addPass(solarBloom);composer.addPass(new OutputPass());
const home=new THREE.Vector3(0,31,43).multiplyScalar(Math.max(1,1.15/(innerWidth/innerHeight)));camera.position.copy(home);controls.target.set(0,0,0);controls.update();scene.add(new THREE.AmbientLight('#ffffff',.035));const sunlight=new THREE.PointLight('#ffffff',3.141592653589793,0,0);scene.add(sunlight);
...
const timeFormat=new Intl.DateTimeFormat('pl-PL',{hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false,timeZone:'UTC'});
let clockShown='';
...
function addView(b){const group=new THREE.Group();scene.add(group);let geo=sphere;if(b.key==='comet'||b.key==='fragment'&&b.irregular)geo=cometNucleusGeometry(b.id,12);else if(b.irregular){geo=new THREE.IcosahedronGeometry(1,3);const a=geo.attributes.position;for(let i=0;i<a.count;i++){const x=a.getX(i),y=a.getY(i),z=a.getZ(i),f=1+.12*Math.sin(x*18+y*13+z*8)+.05*Math.sin(x*37+y*29+z*23);a.setXYZ(i,x*f*1.3,y*f*.78,z*f)}geo.computeVertexNormals()}
 const mat=b.key==='sun'?new THREE.MeshBasicMaterial({color:new THREE.Color('#ffffff').multiplyScalar(24),toneMapped:true}):new THREE.MeshStandardMaterial({map:textures[b.textureKey||b.key]||textures.moon,color:b.textureKey?b.color:b.key==='moon'?b.color:b.key==='blackhole'?'#000000':'#ffffff',roughness:1,metalness:0});if(b.key==='blackhole')mat.map=null;naturalColorMaterial(mat,b.key);
...
function showSpawner(presetId='comet'){
 ...
 function updateDimensions(){
  ...
  document.querySelector('#physical-summary').textContent=validDimensions(m,r)?`${(m/SOLAR_MASS).toLocaleString('pl-PL',{maximumSignificantDigits:4})} mas Słońca · średnica ${(2*r).toLocaleString('pl-PL',{maximumSignificantDigits:5})} km`:'';
 }
 function fillPreset(){
