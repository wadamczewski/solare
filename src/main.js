...
function syncTimeDock(){const special=lightFlight||blackHoleFall;const state=`${paused}:${speed}:${lightFlight?.rate??0}:${blackHoleFall?.clock?.rate??0}:${compressed}:${!!solarDeath}:${systemMode?.id??''}:${surfaceView?.bodyId??''}`;
 ...
 collisionCourseButton.disabled=!!systemMode||!!surfaceView;eclipseButton.disabled=!!systemMode||!!surfaceView;eventTimelineButton.disabled=!!systemMode||!!surfaceView;centralStarInput.disabled=!!systemMode;
...
eventTimelineButton.onclick=showEventTimeline;
