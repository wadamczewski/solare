...
import {isShapeGeometry,shapeGeometry,updateShapeLod} from './scene-lod.js';
...
import {collisionScenarios,collisionLaunchState,scenarioCollisionReady} from './collision-scenarios.js';
...
function radius(b){if(lightFlight){if(flightTrueScale)return b.radius/AU*6;if(b.key==='sun')return .12;if(b.key==='neutron-star')return .04;if(b.parent)return .008;return .018+.095*Math.pow(b.radius/69911,.6)}if(!compressed)return b.radius/AU*6;if(b.key==='sun')return 1.02;if(b.key==='blackhole')return .5*Math.pow(b.mass,.15);if(b.key==='neutron-star')return .055;if(b.parent)return b.irregular?.029:Math.max(.035,b.radius/22000);if(b.key==='comet')return .055;if(b.key==='fragment')return .035+.16*Math.pow(b.radius/69911,.5);return .10+.57*Math.pow(b.radius/69911,.62)}
const sphere=shapeGeometry('high');
function addView(b){const group=new THREE.Group();scene.add(group);let geo=shapeGeometry('medium');if(b.key==='comet'||b.key==='fragment'&&b.irregular)geo=cometNucleusGeometry(b.id,12,b.cometProfile);else if(b.irregular){geo=new THREE.IcosahedronGeometry(1,3);const a=geo.attributes.position;for(let i=0;i<a.count;i++){const x=a.getX(i),y=a.getY(i),z=a.getZ(i),f=1+.12*Math.sin(x*18+y*13+z*8)+.05*Math.sin(x*37+y*29+z*23);a.setXYZ(i,x*f*1.3,y*f*.78,z*f)}geo.computeVertexNormals()}
 const textureKey=b.textureKey||b.key;const surfaceMap=textures[textureKey]||(b.textureKey?loadTexture(textureKey,loaded=>{const material=views.get(b.id)?.mesh.material;if(material){material.map=loaded;material.color.set('#ffffff');material.needsUpdate=true}}):null);const mat=b.key==='sun'?new THREE.MeshBasicMaterial({color:new THREE.Color('#ffffff').multiplyScalar(24),toneMapped:true}):new THREE.MeshStandardMaterial({map:b.key==='moon'?null:surfaceMap,color:surfaceMap?'#ffffff':b.key==='moon'?b.color:b.key==='blackhole'?'#000000':b.key==='neutron-star'?'#d6efff':'#ffffff',roughness:b.key==='neutron-star'?.34:1,metalness:0,emissive:b.key==='neutron-star'?'#2570a8':'#000000',emissiveIntensity:b.key==='neutron-star'?.75:0});if(b.key==='blackhole')mat.map=null;if(b.key==='comet'){mat.map=null;applyCometAppearance(mat,b.cometProfile)}if(b.textureKey&&!surfaceMap){mat.map=null;mat.color.set(b.gas?'#b0aaa0':'#77736c')}naturalColorMaterial(mat,b.key);applyMoonAppearance(mat,b,moonMaps);makeOpaqueSurface(mat);
 const axis=new THREE.Group();axis.rotation.z=THREE.MathUtils.degToRad(b.tilt);group.add(axis);const mesh=new THREE.Mesh(geo,mat);mesh.castShadow=participatesInSolarShadow(b);mesh.receiveShadow=participatesInSolarShadow(b);axis.add(mesh);mesh.userData.id=b.id;const spots=b.key==='sun'?createSolarSpots():null;if(spots)mesh.add(spots);
 ...
 if(['saturn','uranus'].includes(b.key)){const ringGeo=new THREE.RingGeometry(1.35,b.key==='uranus'?1.9:2.35,160,6);const arr=ringGeo.attributes.position,colors=[];for(let i=0;i<arr.count;i++){const r=Math.hypot(arr.getX(i),arr.getY(i)),f=.55+.28*Math.sin(r*100)+.13*Math.sin(r*270);const c=new THREE.Color(b.key==='uranus'?'#555555':'#bcb5a5').multiplyScalar(f);colors.push(c.r,c.g,c.b)}ringGeo.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));const ring=new THREE.Mesh(ringGeo,new THREE.MeshStandardMaterial({vertexColors:true,side:THREE.DoubleSide,transparent:true,opacity:b.key==='uranus'?.25:.65}));ring.castShadow=true;ring.receiveShadow=true;ring.rotation.x=Math.PI/2;mesh.add(ring)}const blackHoleVisual=b.key==='blackhole'?createBlackHoleVisual():null;if(blackHoleVisual)mesh.add(blackHoleVisual.group);const neutronStarVisual=b.key==='neutron-star'?createNeutronStarVisual(b):null;if(neutronStarVisual)mesh.add(neutronStarVisual.group);
 const orbit=createOrbitRibbon({color:b.color,opacity:b.parent?.13:.3});scene.add(orbit);
 const trailGeo=new THREE.BufferGeometry();trailGeo.setAttribute('position',new THREE.BufferAttribute(new Float32Array(512*3),3).setUsage(THREE.DynamicDrawUsage));trailGeo.setDrawRange(0,0);const trail=new THREE.Line(trailGeo,new THREE.LineBasicMaterial({color:b.color,transparent:true,opacity:.3}));trail.frustumCulled=false;scene.add(trail);views.set(b.id,{group,axis,mesh,halo,spots,blackHoleVisual,neutronStarVisual,orbit,trail,history:[],lodLevel:b.irregular?null:'medium',irregular:!!b.irregular});}
...
bs.forEach(addView);applySolarBrightness();scheduleIdle(deferMoonMaps);
...
const asteroidBelt=createAsteroidBelt({random:asteroidRandom});asteroidBelt.update(elapsed,mapped,compressed);scene.add(asteroidBelt.mesh);
...
function launchCollisionScenario(id){
 ...
 projectile.name=`${preset.name} → ${target.name}`;projectile.scenarioId=id;projectile.collisionScenario={targetId:target.id,launchElapsed:elapsed,minDurationDays:state.impactDays};bs.push(projectile);addView(projectile);selected=projectile.id;paused=false;updateOrbits();focusBody(projectile.id,{keepPanel:true});
}
...
function handleCollisions(previous){const oldSelected=selected,oldFollow=follow,wasPanelVisible=!panel.hidden;const visual=compressed?captureCollisionView(bs,displayed,radius):null;const events=resolveCollisions(bs,{contactTest:visual?(a,b)=>scenarioCollisionReady(a,b,elapsed)&&viewContact(a,b,visual,previous):null});if(!events.length)return;preview.clear();for(const event of events){const size=compressed?Math.max(.12,...event.sourceIds.map(id=>views.get(id)?.mesh.scale.x||.12)):event.radius/AU*6;const visualSources=visual&&event.sourceIds.map(id=>visual.get(id)).filter(Boolean);const impactPosition=visualSources?.length?vector(visualSources[0].p).lerp(vector(visualSources[1]?.p||visualSources[0].p),.5):mapped(event.p);impactEffects.add(event,impactPosition,size);for(const id of event.sourceIds){const body=bs.find(b=>b.id===id),view=views.get(id);if(body&&view){if(!view.irregular&&!view.hasDamageGeometry){view.mesh.geometry=shapeGeometry('high');view.lodLevel='high'}applyImpactDamage(view,body)}}const selectedReplacement=Object.hasOwn(event.replacements,oldSelected)?event.replacements[oldSelected]:oldSelected,followReplacement=Object.hasOwn(event.replacements,oldFollow)?event.replacements[oldFollow]:oldFollow;event.removed.forEach(disposeView);for(const id of event.added)addView(bs.find(b=>b.id===id));selected=selectedReplacement;follow=followReplacement;}clearTrails();updateOrbits();if(selected&&wasPanelVisible)showBody();}
