...
- **Ring texture:** Saturn's and Uranus's ring bands (`src/planet-rings.js`) are filled with a five-octave radial noise (`ringGrain`) standing in for the countless individual ice and rock particles a real ring is built from, rather than the two clean sine waves the disc previously showed - a real stellar-occultation profile is dense, irregular grain at every scale, not a printed stripe. The noise is a function of radius only, never angle: the ring mesh is a child of the planet's own spinning mesh, so any azimuthal variation would incorrectly appear to rotate once per planet-day instead of staying fixed the way a ring's broad structure actually does.
- **Ring fragments:** a close-up layer (`src/ring-particles.js`) adds a few thousand individually orbiting fragments on top of that texture, each on its own real Keplerian orbit around the planet rather than sharing one flat rotation - an inner fragment genuinely outruns an outer one, the same differential rotation Cassini measured directly in Saturn's rings. Radii are sampled with each band's own real optical depth as a weight, so fragments cluster densely in the B ring and are almost absent from the Cassini Division and Uranus's wide true gaps, matching where the ring actually has material rather than filling every band equally. Fragments join the planet's tilt group, never its spinning mesh, so they do not inherit the planet's own axial spin; the layer only computes and renders once the camera is close enough for individual fragments to read as more than the textured disc underneath, so it costs nothing at system-map distance.
- **Saturn's small moons:** alongside the eight round Saturn moons already simulated, 15 further real, Cassini-characterised moons are now full N-body bodies, landable in surface view like any other: the ring shepherds and co-orbitals threaded through and just outside the rings (Pan, Daphnis, Atlas, Prometeusz, Pandora, Epimeteusz, Janus, Aegaeon), the Alkyonides embedded in the diffuse dust ring their own collisions help supply (Methone, Anthe, Pallene), and the Tethys/Dione trojans that share those two moons' exact orbits 60° ahead of and behind them (Telesto, Kalipso, Helena, Polideukes) - 293 Saturn moons are now known, but these 23 are the ones well-characterised enough (measured or reliably size-derived mass, radius, and orbit) to simulate honestly as individual bodies. None has a published, coordinate-mapped named surface feature the way the round moons do, so their surface view uses the same honest sub-planetary-point fallback already used for Proteus and Nereid rather than an invented crater. Adding them also lowered the floor on the minimum displayed size a moon can shrink to (`radius()` in `src/main.js`): at the old floor, every sub-hydrostatic moonlet was clamped to the same displayed size, and Atlas and Daphnis - only about 1,200 km apart in real orbital radius - ended up with compressed spheres that touched despite never coming near each other in reality; the lower floor keeps these rubble-pile moonlets legitimately tiny next to Saturn's round moons instead.
- **Saturn's irregular-moon swarm:** the remaining ~268 known Saturn moons are small, distant, individually uncharacterised captured objects - real, but none with a measured size worth an individual simulated body. `src/irregular-moon-swarm.js` renders them as a decorative population of 268 instanced fragments (never part of the gravity simulation), sorted into the three real dynamical families astronomers group them into: the prograde Inuit (39, inclination 43-51°) and Gallic (19, inclination 34-41°) families, and the far larger retrograde Norse family (210, inclination 136-178°, by far most of Saturn's known moons). Each fragment's semi-major axis, eccentricity and inclination are drawn from that family's own real measured range rather than invented, and its position each frame is a genuine eccentric Keplerian orbit (reusing `eccentricAnomaly` from `src/ephemeris.js`, the same solver the real planets use) - a Norse-group fragment's retrograde motion falls directly out of its inclination past 90°, the same way it does for a real orbit, rather than a separate sign flip. Fragments follow Saturn's own current position through the same real-to-scene distance compression every simulated moon already uses (`satelliteOffset`, now shared from `src/scene-scale.js` rather than duplicated), so they sit at a properly exaggerated-but-consistent distance from Saturn instead of either overlapping it or scattering off screen. Phoebe, the family's largest and best-characterised member, still isn't a full simulated body: its real ~175° inclination doesn't fit the one shared inclination `initialSystem()` gives every other Saturn moon, so it remains part of the decorative swarm rather than being shoehorned into that model - left for a future commit that gives it its own treatment.
...
These labels are DOM overlays rather than WebGL geometry. Unlike the scene, which renders every frame, they are normally redrawn only every sixth frame while the camera is stationary. Camera orientation is now checked for a meaningful change using the quaternion dot product (`quaternionChanged` in `src/sky-labels.js`), and a changed orientation updates labels immediately. The six-frame cadence is retained only as a fallback while the camera is still, for example when the surface-view horizon moves with simulated time. A second, separate case had its own version of the same problem: the surface view's HUD, direction radar, and sky markers were redrawn on the same fixed six-frame cadence. That gap is imperceptible at a slow tempo, but at a fast one the same six frames can span a noticeable arc of sky, so marker motion looked clipped every few frames instead of smooth. The cadence now shortens with simulation speed—every sixth frame below 2 days/s, every third between 2 and 100, and every frame at 100 and above—the same mechanism that already drives the orbit-line refresh below.

## Model and limits of accuracy

Physics uses AU, solar masses, and days, independently of visual planet enlargement. All 47 default bodies (the Sun, eight planets, and 38 selected moons) and added objects mutually interact through Newtonian gravity. The integrator is velocity Verlet, with a step limited by dynamical time and close approaches. At physical radii, collisions distinguish merging, fragment ejection, disruptive impact, grazing collision, and absorption. Mass and linear momentum are conserved. The initial system is barycentric, and planet parameters use JPL tables.

**Physics vectors.** The *Physics vectors* layer draws a velocity (blue) and gravity (gold) arrow on every body in view that moves relative to its primary (its planet, or the dominant star; in a binary each star is measured against its companion), sized on screen so dozens stay legible; the selected body's are longer and opaque, and its numbers appear in the physics card (`src/education-vectors.js`).
...
- Lines for 89 constellations, off by default through the **Constellations** switch in the Simulation panel. They are a human-made orientation overlay rather than physical objects, so they are not enabled automatically. Hovering hits a wide margin along the figure's own line, not just its thin geometry, and the highlight fades in and out smoothly instead of snapping.
...
Each body's detail panel also links to Wikipedia and loads a small photo gallery from it. The search term is the object's own proper name in whichever of the four interface languages is currently selected, reusing the same pl/en/de/es name table that already labels every planet, moon, and named deep-sky object elsewhere in the interface, rather than a single fixed English or IAU identifier; switching language while a panel is open re-runs the lookup immediately. Objects whose name is the same in every language — most exoplanet, pulsar, and black-hole designators — keep one identifier regardless of language, and a handful of names with no matching interface row (the Crab and Vela pulsars, 1 Ceres, 1P/Halley, the Double Cluster's two halves, Vega's German "Wega") fall back to a small dedicated table. One interface translation is deliberately excluded from the reuse: a custom black hole's own name translates to the English UI label "Custom black hole", which is not the title of any encyclopedia article, so it keeps the generic "Black hole" fallback instead.
