...
Grunt pod nogami cieniuje się z rzeczywistego nachylenia, nie z przybliżenia ekranowego Three.js: ten drugi liczy różnicę wysokości między sąsiednimi pikselami ekranu, więc gaśnie do płaskiego obrazu, gdy jeden teksel mapy wysokości (liczonej w tysiącach kilometrów) pokrywa mniej niż jeden piksel — a to właśnie sytuacja stania na powierzchni. Marsa i Księżyc mają normalną mapę policzoną offline z ich rzeczywistych danych wysokościowych (MOLA, LOLA — `tools/build-surface-normals.mjs`), reszta ciał (w tym Ziemia, bez własnej mapy wysokości) dostaje tę samą technikę policzoną z proceduralnego pola reliefu. Do tego dochodzi druga, dużo drobniejsza warstwa ziarna — ogólny kamienisto-piaszczysty wzór, niezwiązany z żadnym konkretnym ciałem, powielany kilkaset razy po powierzchni i mieszany z trzech rzutów (bez szwu przy biegunach), żeby grunt tuż pod obserwatorem nie wyglądał jak rozmyte powiększenie globalnej mapy.

Widok z powierzchni ma sekcję „Znane miejsca” (`src/surface-places.js`): realne, nazwane miejsca — Olympus Mons, Krater Tycho, Kraken Mare i tak dalej, wg oficjalnego nazewnictwa IAU — z których kliknięcie przenosi w to miejsce na tym samym ciele, zamiast szerokości/długości zero. Sekcja mieszka w panelu widoku z powierzchni, nie w ogólnym panelu szczegółów ciała, bo dotyczy konkretnie tego, na czym się aktualnie stoi. Stojąc na powierzchni, te same ciała Układu Słonecznego, które śledzi już radar, oraz — tam gdzie włączony jest istniejący przełącznik obiektów głębokiego nieba — także one, dostają teraz znacznik z nazwą i obrysem narysowany wprost nad ich rzeczywistą pozycją na renderowanym niebie (`src/sky-labels.js`), nie tylko na radarze czy na liście obiektów.

Kliknięcie znanego miejsca patrzy też w dół, ku gruntowi, zamiast szukać czegoś do obejrzenia na niebie jak zwykłe wejście w widok z powierzchni: skoro miejsce jest cechą samego gruntu, kamera pochyla się o 18° poniżej horyzontu, więc grunt wypełnia większość kadru, ale nie cały — nad horyzontem zostaje pasek otwartego nieba. Na Ziemi taki wybór jest też nadrzędny wobec lokalizacji urządzenia: geolokalizacja przeglądarki (`navigator.geolocation.watchPosition`) nadal działa w tle, ale jeśli obserwator stoi w wybranym ręcznie miejscu — klikniętym znanym miejscu albo przesuniętym suwakiem szerokości/długości — kolejny odczyt z urządzenia go nie nadpisuje. Panel jasno nazywa, co widać: „Wybrana lokalizacja” dla ręcznie ustawionej pozycji, „Lokalizacja urządzenia” tylko wtedy, gdy pozycja rzeczywiście pochodzi z bieżącego odczytu GPS/sieci. Zmiana ciała w widoku z powierzchni zaczyna od nowa i ponownie ufa urządzeniu.

Panel szczegółów dowolnego ciała z listą znanych miejsc ma też własny przełącznik „Punkty orientacyjne” (domyślnie wyłączony): włączony, zaznacza te same miejsca wprost na małym obracającym się podglądzie ciała, w ich rzeczywistym położeniu, żeby zobaczyć, gdzie na planecie leży to, co najciekawsze, bez wchodzenia w widok z powierzchni. Pozycja znacznika liczona jest tą samą kalibracją równoprostokątną, której widok z powierzchni używa do orientacji gruntu (`src/surface-texture-frame.js`, `localSurfacePoint`), więc znacznik obraca się razem z ciałem — jest dzieckiem tej samej siatki. Ten sam komplet znaczników trafia też wprost na prawdziwą siatkę ciała w głównej scenie, więc kliknięcie „Śledź” i przybliżenie kamery pokazuje te same punkty na dużym, śledzonym ciele, nie tylko na maleńkim podglądzie w panelu — przełącznik i lista miejsc są wspólne dla obu. Te znaczniki w głównej scenie są też interaktywne, tak samo jak ciała, obiekty głębokiego nieba i gwiazdozbiory: najechanie pokazuje etykietę z nazwą miejsca, a kliknięcie przenosi wprost w widok z powierzchni stojący dokładnie w tym miejscu, patrząc w stronę gruntu — to samo, co kliknięcie tej pozycji na liście „Znane miejsca” w widoku z powierzchni.

Radar w rogu ekranu (`src/surface-radar.js`) to osobna, mała scena Three.js — kula zamiast płaskiego pierścienia — więc pokazuje pełny kierunek 3D śledzonego ciała naraz: i azymut, i wysokość nad horyzontem, zamiast samego namiaru. Kopuła sama się nie obraca — jest symetryczna względem osi, więc z każdego kierunku patrzenia wygląda tak samo — obracają się tylko znaczniki, przeliczane co odświeżenie z bieżącego kierunku patrzenia czystą funkcją w `src/surface-compass.js` (`relativeBearing`, `sphericalToRadar`). Ciało poniżej horyzontu nadal jest widoczne, przyciemnione, przez półprzezroczystą kopułę — więc widać od razu, czy w ogóle warto się odwrócić, bez pełnego obrotu.

Znacznik z nazwą i obrysem dla obiektu głębokiego nieba towarzyszy teraz punktom na kopule nieba (`src/sky.js`) wszędzie tam, gdzie są rysowane, a nie tylko w widoku z powierzchni: w swobodnym widoku orbitalnym, w gotowych układach gwiazdowych, w locie z prędkością światła i przy wpadaniu w czarną dziurę — zawsze, gdy włączony jest przełącznik „Obiekty głębokiego nieba”. Tylko stojąc na powierzchni znaczniki nadal są ograniczone do tego, co ponad horyzontem — bo tam grunt rzeczywiście zasłania resztę nieba; w każdym innym widoku liczy się jedynie to, co akurat mieści się przed kamerą.

Te znaczniki są nakładką DOM, nie częścią sceny WebGL, więc — inaczej niż sama scena, przerysowywana co klatkę bez wyjątku — opłaca się je przemalowywać tylko co szóstą klatkę, gdy kamera stoi w miejscu. Ten sam, stały odstęp obowiązywał jednak także wtedy, gdy kamera się obracała: podczas przeciągania ekranu scena podążała za kursorem natychmiast, a etykiety obiektów głębokiego nieba potrafiły spóźniać się o te kilka klatek, więc widocznie „ślizgały się” za resztą obrazu zamiast trzymać się swoich punktów. Teraz przemalowanie następuje natychmiast, gdy tylko orientacja kamery rzeczywiście się zmieni — o więcej niż drobny, czysto zmiennoprzecinkowy szum (`quaternionChanged` w `src/sky-labels.js`, licząc kąt między dwoma kwaternionami z ich iloczynu skalarnego) — a stały odstęp co sześć klatek zostaje już tylko jako zapasowe odświeżenie na czas bezruchu kamery (np. gdy horyzont w widoku z powierzchni przesuwa się z upływem czasu symulacji).
...
Scena trzyma ekliptykę w płaszczyźnie XZ, a biegun ekliptyki na osi +Y. Odwzorowanie jest obrotem, nie odbiciem: trzecia współrzędna zmienia znak. Sama zamiana dwóch osi byłaby permutacją nieparzystą, czyli lustrem — planety krążyłyby zgodnie z ruchem wskazówek zegara patrząc od bieguna północnego ekliptyki, a każdy gwiazdozbiór wyszedłby jako własne odbicie. Test na zachowanie iloczynu wektorowego pilnuje tego znaku.

Widok „Czytelny” powiększa promienie i nieliniowo skraca odległości. „Rzeczywista skala” przywraca proporcje przestrzeni i promieni. Cienkie orbity to chwilowe oskulacyjne rozwiązania dwuciałowe, aktualizowane z bieżących stanów; ślady pokazują faktyczny przebieg symulacji. Przełącznik „Orbity” w panelu Symulacja pozwala je całkiem ukryć, gdy przeszkadzają w oglądaniu samych ciał i śladów; domyślnie są widoczne. Przy bardzo bliskich spotkaniach i szybkim tempie limit pracy klatki spowalnia upływ symulacji, zamiast zwiększać krok i destabilizować układ.
...
- 118 216 gwiazd do jasności 14 mag, z rektascensją, deklinacją, jasnością i wskaźnikiem barwy B-V. Barwa gwiazdy wynika z B-V przez wzór Ballesterosa (2012) na temperaturę i przybliżenie ciała doskonale czarnego. Rozmiar rośnie z jasnością powoli, resztę niesie luminancja HDR, dzięki czemu najjaśniejsze gwiazdy rozkwitają w bloomie. Katalog sięga daleko za granicę gołego oka (6 mag w ziemskiej atmosferze) celowo: powietrze ma ekstynkcję i światło miejskie, a przestrzeń kosmiczna, w której cała ta symulacja się dzieje, nie ma żadnego z nich (ten sam argument, co przy normalizacji jasności gwiazd w `src/sky.js`), więc prawdziwy obserwator w kosmosie rozdziela coraz słabsze gwiazdy bez wyraźnej granicy zamiast trafiać na twardy próg.
- Droga Mleczna dwiema warstwami: gładka mapa luminancji zrasteryzowana z pięciu poziomów izofot survey'u, w rozdzielczości natywnej siatki roboczej (2880×1440, tyle ile mniej więcej niosą same wierzchołki wielokątów izofot) zamiast wcześniejszego decymowania do 2048×1024, oraz gęsta chmura 500 000 punktów na wierzchu zamiast poprzednich 110 000. Pas fizycznie jest światłem nierozdzielonych gwiazd, więc mapa niesie poświatę, a punkty przywracają ziarno tych gwiazd, które się rozdzielają - w przestrzeni kosmicznej, bez atmosfery między obserwatorem a pasem, rozdziela się ich znacznie więcej niż z powierzchni Ziemi.
- 59 jasnych obiektów głębokiego nieba: Galaktyka Andromedy, Galaktyka Trójkąta, oba Obłoki Magellana, Mgławica Oriona, Plejady, Omega Centauri, 47 Tucanae, Galaktyka Wir, Mgławica Pierścień, Mgławica Krab, Galaktyka Sombrero i inne, w skali odpowiadającej ich rzeczywistej rozciągłości kątowej i z jasnością powierzchniową malejącą wraz z rozmiarem. Trzon to d3-celestial's własna, ręcznie dobrana lista „bright"; 28 kolejnych to reszta katalogu Messiera, dobrana za rzeczywistą sławę (ma własną, powszechnie znaną nazwę), nie tylko za jasność.
