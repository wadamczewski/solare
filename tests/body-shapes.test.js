...
import {bodyAxes,hasMeasuredIrregularShape,largestAxis,measuredIrregularGeometry,shapeSources,surfaceRadialScale} from '../src/body-shapes.js';
...
test('Deimos uses its measured rounded triaxial envelope instead of the generic faceted rock',()=>{
 ...
 assert.ok(axes[0]>1.18&&axes[1]<.85,`unexpected Deimos axes ${axes}`);
 assert.ok(largestAxis(deimos)<1.25);
 assert.ok(geometry?.attributes.position.count>1000);
 geometry.dispose();
});

test('the polar surface of an oblate planet uses its polar radius, not its equatorial radius',()=>{
 const saturn=initialSystem().find(body=>body.key==='saturn');
 const equator=surfaceRadialScale(saturn,0,0),northPole=surfaceRadialScale(saturn,90,0);
 assert.ok(equator>1.03,`Saturn equator ${equator}`);
 assert.ok(northPole<.98,`Saturn pole ${northPole}`);
 assert.ok(equator>northPole*1.09,`Saturn oblateness ${equator}/${northPole}`);
});
